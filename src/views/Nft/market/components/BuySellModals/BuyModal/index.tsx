...
const BuyModal: React.FC<BuyModalProps> = ({ nftToBuy, onDismiss }) => {
  ...
  const [paymentCurrency, setPaymentCurrency] = useState<PaymentCurrency>(PaymentCurrency.PLS)
  ...
  const walletBalance = paymentCurrency === PaymentCurrency.PLS ? formattedBnbBalance : formattedWbnbBalance
  const walletFetchStatus = paymentCurrency === PaymentCurrency.PLS ? bnbFetchStatus : wbnbFetchStatus

  const notEnoughBnbForPurchase =
    paymentCurrency === PaymentCurrency.PLS
  ...
  useEffect(() => {
    if (bnbBalance.lt(nftPriceWei) && wbnbBalance.gte(ethersToBigNumber(nftPriceWei)) && !isPaymentCurrentInitialized) {
      setPaymentCurrency(PaymentCurrency.WPLS)
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    ...
    onApproveSuccess: async ({ receipt }) => {
      toastSuccess(
        t('Contract approved - you can now buy NFT with WPLS!'),
    ...
    onConfirm: () => {
      ...
      if (paymentCurrency === PaymentCurrency.PLS) {
        return callWithGasPrice(nftMarketContract, 'buyTokenUsingPLS', [nftToBuy.collectionAddress, nftToBuy.tokenId], {
          value: payAmount,
        })
      }
  ...
  const continueToNextStage = () => {
    if (paymentCurrency === PaymentCurrency.WPLS && !isApproved) {
