...
const Header: React.FC<HeaderProps> = ({ collection }) => {
  ...
  const { totalSupply, numberTokensListed, totalVolumePLS, banner, avatar } = collection
  const { t } = useTranslation()
  const { pathname, hash } = useLocation()

  const volume = totalVolumePLS
    ? parseFloat(totalVolumePLS).toLocaleString(undefined, {
      ...
      <MarketPageHeader>
        ...
        >
          <StatBox>
            ...
            <StatBoxItem title={t('Vol. (%symbol%)', { symbol: 'PLS' })} stat={volume} />
          </StatBox>
        </MarketPageTitle>
      </MarketPageHeader>
      <Container>
