...
import { ARCHIcon, Flex, IconButton, Link, Button, useModal } from '@pancakeswap/uikit'
        ...
        <Flex display="inline-flex">
          {accountPath && (
            <StyledIconButton
              target="_blank"
              as="a"
              href={getaRCHLink(accountPath, 'address')}
              alt={t('ViewExplorer for user address')}
            >
              <ARCHIcon width="20px" color="primary" />
            </StyledIconButton>
          )}
        </Flex>
      )
    ...
    return (
      <Flex flexDirection="column" mb={[16, null, 0]} mr={[0, null, 16]}>
        {accountPath && profile?.username && (
          <Link href={getaRCHLink(accountPath, 'address')} external bold color="primary">
