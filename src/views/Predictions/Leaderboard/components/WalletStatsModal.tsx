      ...
      <ModalHeader background={theme.colors.gradients.bubblegum}>
        <Flex alignItems="center" style={{ flex: 1 }}>
          ...
          <Box>
            ...
            <ExternalLink href={getaRCHLink(address, 'address')}>{truncateHash(address)}</ExternalLink>
          </Box>
        </Flex>
          ...
          >
            <Box>
              ...
              {isLoading ? (
                ...
                <NetWinnings
                  amount={result?.netPLS}
                  textPrefix={result?.netPLS > 0 ? '+' : ''}
                  textColor={result?.netPLS > 0 ? 'success' : 'failure'}
                  alignItems="flex-end"
                />
              )}
            </Box>
