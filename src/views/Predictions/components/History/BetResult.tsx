      ...
      <StyledBetResult>
        ...
        {bet.claimed && bet.claimedHash && (
          <Flex justifyContent="center">
            <LinkExternal href={getaRCHLink(bet.claimedHash, 'transaction')} mb="16px">
              {t('View onExplorer')}
            </LinkExternal>
          </Flex>
        )}
        ...
        <Flex alignItems="center" justifyContent="space-between" mb="16px">
          ...
          <Text>{`${formatBnb(bet.amount)} PLS`}</Text>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between">
          <Text bold>{isWinner ? t('Your winnings') : t('Your Result')}:</Text>
          <Box style={{ textAlign: 'right' }}>
            <Text bold color={getResultColor()}>{`${isWinner ? '+' : '-'}${formatBnb(payout)} PLS`}</Text>
          ...
          <>
            ...
            <Flex alignItems="start" justifyContent="space-between">
              ...
              <Flex justifyContent="end">
                <Text fontSize="14px" color="textSubtle">{`${formatBnb(returned)} PLS`}</Text>
                <span ref={targetRef}>
