...
const PnlTab: React.FC<PnlTabProps> = ({ hasBetHistory, bets }) => {
  ...
  const bnbBusdPrice = usePLSBusdPrice()
    ...
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatBnb(netResultAmount)} PLS`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgBnbWonIsPositive ? 'success' : 'failure'}>
          {`${avgBnbWonIsPositive ? '+' : ''}${formatBnb(avgBnbWonPerRound)} PLS`}
        </Text>
        <Text small color="textSubtle">
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatBnb(summary.won.bestRound.payout)} PLS`}</Text>
        ...
        <Text bold>{`${formatBnb(avgPositionEntered)} PLS`}</Text>
        <Text small color="textSubtle">
        ...
        <Flex justifyContent="center" mt="24px">
          <Link href={`${getaRCHLink(account, 'address')}#internaltx`} mb="16px" external>
