...
const fetchBnbPrices = async (
  ...
  try {
    const data = await request<PricesResponse>(INFO_CLIENT, PLS_PRICES, {
      block24,
      block48,
      blockWeek,
    })
    ...
    console.error('Failed to fetch PLS prices', error)
    return {
      error: true,
      bnbPrices: undefined,
    }
...
/**
 * Returns PLS prices at current, 24h, 48h, and 7d intervals
 */
