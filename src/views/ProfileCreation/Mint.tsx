...
import { MINT_COST, STARTER_NFT_BUNNY_PSB } from './config'
...
const Mint: React.FC = () => {
  ...
  useEffect(() => {
    const getStarterNfts = async () => {
      ...
      const nfts = STARTER_NFT_BUNNY_PSB.map((bunnyId) => {
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% PSB', { num: formatUnits(MINT_COST) })}
          </Text>
          {starterNfts.map((nft) => {
          ...
          {!hasMinimumCakeRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% PSB is required', { num: formatUnits(MINT_COST) })}
            </Text>
          )}
