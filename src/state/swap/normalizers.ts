  ...
  return data?.token0DerivedBnb.reduce((acc, token0DerivedBnbEntry) => {
    ...
    return [
      ...acc,
      {
        ...
        token0DerivedPLS: token0DerivedBnbEntry.derivedPLS,
        token1DerivedPLS: token1DerivedBnbEntry.derivedPLS,
      },
    ]
  }, [])
}
...
export const normalizeDerivedPairDataByActiveToken = ({
  ...
  pairData?.map((pairPrice) => ({
    ...
    value:
      activeToken === pairPrice?.token0Id
        ? pairPrice.token0DerivedPLS / pairPrice.token1DerivedPLS
        : pairPrice.token1DerivedPLS / pairPrice.token0DerivedPLS,
  }))
