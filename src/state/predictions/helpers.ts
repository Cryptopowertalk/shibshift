...
export const transformBetResponse = (betResponse: BetResponse): Bet => {
  const bet = {
    ...
    claimedPLS: betResponse.claimedPLS ? parseFloat(betResponse.claimedPLS) : 0,
    claimedNetPLS: betResponse.claimedNetPLS ? parseFloat(betResponse.claimedNetPLS) : 0,
...
export const transformUserResponse = (userResponse: UserResponse): PredictionUser => {
  ...
  return {
    ...
    totalPLS: totalPLS ? parseFloat(totalPLS) : 0,
    totalPLSBull: totalPLSBull ? parseFloat(totalPLSBull) : 0,
    totalPLSBear: totalPLSBear ? parseFloat(totalPLSBear) : 0,
    totalBetsClaimed: numberOrNull(totalBetsClaimed),
    totalPLSClaimed: totalPLSClaimed ? parseFloat(totalPLSClaimed) : 0,
    winRate: winRate ? parseFloat(winRate) : 0,
    averagePLS: averagePLS ? parseFloat(averagePLS) : 0,
    netPLS: netPLS ? parseFloat(netPLS) : 0,
  }
}
...
export const getTotalWon = async (): Promise<number> => {
  ...
  const totalPLS = market.totalPLS ? parseFloat(market.totalPLS) : 0
  const totalPLSTreasury = market.totalPLSTreasury ? parseFloat(market.totalPLSTreasury) : 0

  return Math.max(totalPLS - totalPLSTreasury, 0)
}
