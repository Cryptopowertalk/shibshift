...
const getFarmBaseTokenPrice = (
  ...
  // We can only calculate profits without a quoteTokenFarm for DAI/PLS farms
  if (!quoteTokenFarm) {
    return BIG_ZERO
  }
  ...
  // If the farm's quote token isn't DAI or WPLS, we then use the quote token, of the original farm's quote token
  // i.e. for farm PNT - pBTC we use the pBTC farm's quote token - PLS, (pBTC - PLS)
  // from the PLS - pBTC price, we can calculate the PNT - DAI price
  ...
  // Catch in case token does not have immediate or once-removed DAI/WPLS quoteToken
  return BIG_ZERO
}
