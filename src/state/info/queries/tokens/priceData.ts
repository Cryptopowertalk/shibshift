...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      // if its PLS price e.g. `b123` split('t')[1] will be undefined and skip PLS price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
          derivedPLS: prices[priceKey]?.derivedPLS ? parseFloat(prices[priceKey].derivedPLS) : 0,
          priceUSD: 0,
        })
      }
    })

    // Go through PLS USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          const { derivedPLS } = tokenPrices[tokenPriceIndex]
          tokenPrices[tokenPriceIndex].priceUSD = parseFloat(prices[priceKey]?.bnbPrice ?? 0) * derivedPLS
        }
      }
    })
