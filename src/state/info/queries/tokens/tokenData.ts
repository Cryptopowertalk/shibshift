    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          const priceUSD = current ? current.derivedPLS * bnbPrices.current : 0
          const priceUSDOneDay = oneDay ? oneDay.derivedPLS * bnbPrices.oneDay : 0
          const priceUSDWeek = week ? week.derivedPLS * bnbPrices.week : 0
