...
export const useIfoV4Contract = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getIfoV4Contract(address, library.getSigner()), [address, library])
}
...
export const useWrappedPLS = (address: string) => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getwrappedPLSContract(address, library.getSigner()), [address, library])
}
